...
#conduct a forward selection
fit.fwd = regsubsets(Outstate~., data=train_data, nvmax=17, method="forward")
...
#Show the Mallows's Cp statistic, BIC, Adjusted R square value to decide which how many predictors we will use.
par(mfrow=c(2,2))
...
~~~
The model we use include 6 predictors: PrivateYes, Room.Board, PhD, perc.alumni, Expend, Grad.Rate.
...
```{r,echo=TRUE}
library(mgcv)
gam.m1 <- gam(Outstate ~ Private+ poly(Room.Board,2)+ poly(PhD,2) + poly(perc.alumni,2) + poly(Expend,2) + poly(Grad.Rate,2), data = train_data)
gam.m1
...
~~~
out of state tuition increases as room and board costs get higher and also increases as the proportion of alumni who donate increase.
...
mse = mean((test_data$Outstate - preds)^2)
mse
```
...
~~~
The mse is large, the model is not ideal. 
...
Your code:
```{r,echo=TRUE}
gam.m2 = gam(Outstate~Private+poly(Room.Board,4)+poly(PhD,4)+poly(perc.alumni,2)+poly(Expend,4), data=college_train)
gam.m3 = gam(Outstate~Private+poly(Room.Board,4)+poly(PhD,4)+poly(perc.alumni,2)+poly(Expend,4)+Grad.Rate, data=college_train) # Linear Grad rate
gam.m4 = gam(Outstate~Private+poly(Room.Board,4)+poly(PhD,4)+poly(perc.alumni,2)+poly(Expend,4)+poly(Grad.Rate,4), data=college_train) # Spline with 4 degrees
...
~~~
`Grad.Rate` is a non-linear function(spline with degree 2) is needed.
...
fit.6 = lm(mpg ~ poly(displacement ,6) ,data=Auto)
...
~~~
The result shows that there a obvious quadratic relationship between displacement and mpg.
...
beta <- rnorm(p)  # True coefficients
Y <- X %*% beta + rnorm(n)  # Response variable

# Fit multiple linear regression to get true coefficients
true_fit <- lm(Y ~ X)

# Backfitting procedure
coefs_backfit <- rep(0, p)
max_iterations <- 1000
tolerance <- 1e-6
coefs_history <- matrix(NA, nrow = max_iterations, ncol = p)

for (iteration in 1:max_iterations) {
  coefs_history[iteration, ] <- coefs_backfit
  for (j in 1:p) {
    # Residuals excluding the effect of the j-th predictor
    Y_residuals <- Y - X %*% coefs_backfit + X[, j] * coefs_backfit[j]
    # Update the j-th coefficient using simple linear regression
    fit <- lm(Y_residuals ~ X[, j] - 1)  # -1 to omit the intercept
    coefs_backfit[j] <- fit$coefficients
  }
  
  # Check for convergence
  if (max(abs(coefs_history[iteration, ] - coefs_backfit)) < tolerance) {
    message(sprintf("Convergence reached after %d iterations", iteration))
    coefs_history <- coefs_history[1:iteration, ]
    break
  }
}

# Plot the convergence of coefficients
matplot(coefs_history, type = 'l', lty = 1, main = "Convergence of Backfitting Coefficients",
        xlab = "Iteration", ylab = "Coefficient Estimate", col = rainbow(p))
legend("topright", legend = c("True Coefficients", "Backfitting Estimates"), col = c(1, 2), lty = 1:2)
...
plot(dis.grid,preds$fit,col="blue",lwd=3)
lines(dis.grid,preds$fit+2*preds$se,col="red",lwd=3,lty=2)
lines(dis.grid,preds$fit-2*preds$se,col="yellow",lwd=3,lty=2)
```
...
~~~
The RSS decreases from the linear to the cubic model, and increases thereafter. This supports the argument that the cubic model provides the best fit.
...
```{r,echo=TRUE}
library(boot)
cv_poly_degree <- function(data, max_degree = 5) {
  cv_errors <- rep(0, max_degree)
  
  # Cross-validation for polynomial degrees from 1 to max_degree
  for (degree in 1:max_degree) {
    # Create a formula with a polynomial term
    formula <- as.formula(paste('nox ~ poly(dis, ', degree, ')', sep = ''))
    
    # Fit model and perform k-fold cross-validation
    glm_fit <- glm(formula, data = data)
    cv_fit <- cv.glm(data, glm_fit, K = 10)  # 10-fold CV
    
    # Store the cross-validation error
    cv_errors[degree] <- cv_fit$delta[1]
  }
  
  return(cv_errors)
}

# Perform cross-validation for degrees 1 to 5
set.seed(42)  # For reproducibility
max_degree <- 5
cv_errors <- cv_poly_degree(Boston, max_degree)

# Find the optimal degree with the lowest CV error
optimal_degree <- which.min(cv_errors)

# Plot the cross-validation errors for each degree
plot(1:max_degree, cv_errors, type = 'b', xlab = 'Polynomial Degree', ylab = 'CV Error',
     main = 'Cross-Validation Error for Different Polynomial Degrees')
points(optimal_degree, cv_errors[optimal_degree], col = 'red', cex = 1.5, pch = 20)

list(optimal_degree = optimal_degree, cv_errors = cv_errors)
```

Your answer:

~~~
The optimal degree find by cv is 4, which is not cubic.
...
lines(dis.grid, preds$fit+2*preds$se,col="red",lwd=3,lty=2)
lines(dis.grid, preds$fit-2*preds$se,col="green",lwd=3,lty=2)
